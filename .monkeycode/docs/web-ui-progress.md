...
- [ ] **前端调用后端 start API**
  ...
  - 需要：发送 POST 请求到 `/api/v1/crack/start`
  - 传递：用户输入的密码

- [ ] **前端调用后端 stop API**
  - 位置：`index.html` 停止按钮点击事件
  - 需要：发送 POST 请求到 `/api/v1/crack/stop`

- [ ] **前端连接 WebSocket**
  - 需要：建立 `/ws/crack/progress` WebSocket 连接
  - 当前：使用模拟数据更新工作状态
...
- [ ] **前端尝试次数从后端获取**
  - 当前：`cracking-attempts` 显示是模拟增长的
  - 需要：对接 WebSocket 推送的真实数据

- [ ] **成功状态联动**
  - 当前：`view-success` 密码显示固定为 "******"
  - 需要：从后端接收找到的密码和真实统计数据

### 前端工作状态展示优化

- [ ] **初始值从后端获取**
  - 当前：`initWorkStatus()` 使用硬编码值
  - 需要：从 start API 响应中获取真实运行模式和并发数

- [ ] **规则切换逻辑**
  - 当前：前端基于尝试次数阈值模拟规则切换
  - 需要：从后端推送的真实 `rule_id` 映射到规则名称
