...
实现一个在 Windows 系统（Windows 7/10/11）中运行的弱口令枚举暴力破解演示程序。用户输入目标密码后，程序通过动态数量的工作进程/线程并发枚举尝试，直到匹配成功或用户手动终止。

### 原始需求

1. 用户输入目标密码
2. 按 5 种规则顺序枚举（纯数字、字母头+数字、数字+字母尾、小写混合、全字母混合）
3. 根据 CPU 硬件自动分配 Worker 数量，任一找到即终止
4. 显示进度、累计用时，支持提前终止
5. 破解完成后显示总尝试次数和时间
...
| `93e8774` | Phase 2 优化：动态多进程/多线程调度、Worker 文件日志、CLI 刷新修复 |
| `5d69aeb` | 修复 Windows 多进程启动问题：恢复 freeze_support()、增加诊断日志 |
